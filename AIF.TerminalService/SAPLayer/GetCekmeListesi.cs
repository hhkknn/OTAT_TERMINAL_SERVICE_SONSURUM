    ...
    public class GetCekmeListesi
    {
        public Response getCekmeListesi(string dbName, string baslangicTarihi, string bitisTarihi, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    ...
                    var query = "Select * from (SELECT T0.\"DocEntry\",T0.\"U_MusteriKodu\",T0.\"U_MusteriAdi\",\"U_Aciklama\",(Select COUNT(T1.\"DocEntry\") from \"@AIF_WMS_SIPKAR1\" as T1 WITH (NOLOCK) where T1.\"DocEntry\" = T0.\"DocEntry\"  and T1.\"U_Gorunur\" = 'Y') as \"SatirSayisi\" FROM \"@AIF_WMS_SIPKAR\" AS T0 WITH (NOLOCK) WHERE T0.\"U_OnayDurumu\" = 'O' AND(\"U_TerminalGizle\" <> 'Y' OR \"U_TerminalGizle\" IS NULL)) as tbl where tbl.\"SatirSayisi\" > 0";
                    try
                    {
        ...
        public Response getCekmeListesiKoliDetaylari(string dbName, string docEntry, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    var query = "SELECT T1.\"U_SiparisNo\",T1.\"U_SatirNo\", T1.\"U_KoliAdedi\", T1.\"U_KoliIciAdedi\", T1.\"U_ToplamMiktar\" FROM \"@AIF_WMS_KOLIDTY\" T0 WITH (NOLOCK) INNER JOIN \"@AIF_WMS_KOLIDTY1\" T1 WITH (NOLOCK) ON T0.\"DocEntry\" = T1.\"DocEntry\" WHERE T0.\"U_BelgeNo\" ='" + docEntry + "'";
        ...
        public Response getToplamaListesi(string dbName, string baslangicTarihi, string bitisTarihi, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    ...
                    string query = "  select DISTINCT tbl.* from (Select  T0.\"U_BelgeNo\",(SELECT Count(\"DocEntry\") FROM \"@AIF_WMS_KNTYNR1\" AS T98 WHERE T98.\"U_PaletNo\" = T0.\"U_PaletNo\") AS \"KonteynerVarmi\",T1.\"CardCode\" as \"MusteriKodu\",T1.\"CardName\" as \"MusteriAdi\",T3.\"U_Aciklama\" from \"@AIF_WMS_TOPLANAN\" as T0 WITH (NOLOCK) INNER JOIN ORDR AS T1 WITH (NOLOCK) ON T0.\"U_SiparisNumarasi\" = T1.\"DocEntry\" INNER JOIN RDR1 WITH (NOLOCK) AS T2 ON T1.\"DocEntry\" = T2.\"DocEntry\" and T2.\"LineNum\" = T0.\"U_SiparisSatirNo\" LEFT JOIN \"@AIF_WMS_SIPKAR\" AS T3 WITH (NOLOCK) ON T3.\"DocEntry\" = T0.\"U_BelgeNo\" ) as tbl INNER JOIN RDR1 WITH (NOLOCK) AS T99 ON tbl.\"U_BelgeNo\" = T99.\"DocEntry\" INNER JOIN ORDR AS T98 WITH (NOLOCK) ON T98.\"DocEntry\" = T99.\"DocEntry\" WHERE tbl.\"KonteynerVarmi\" <= 0 and T98.\"DocDueDate\" >= '" + baslangicTarihi + "'  and  T98.\"DocDueDate\" <= '" + bitisTarihi + "'";
        ...
        public Response getToplamaListesiDetay(string dbName, string docEntry, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    ...
                    string query = "select * from (Select T0.\"U_SiparisNumarasi\" as \"SiparisNumarasi\",T1.\"DocDate\" as \"SiparisTarihi\",T1.\"DocDueDate \" as \"TeslimatTarihi\",T0.\"U_SiparisSatirNo\" as \"SiparisSatirNo\",T2.\"ItemCode\" as \"UrunKodu\",T2.\"Dscription\" as \"UrunTanimi\",T2.\"Quantity\" as \"ToplamSiparisMiktari\", (T2.\"Quantity\" - T2.\"OpenQty\") as \"SevkSipMiktari\", (T2.\"OpenQty\" - T0.\"U_Miktar\") AS \"AcikSiparisMiktari\",T2.\"WhsCode\" as \"SiparisDepoKodu\",T0.\"U_Miktar\" as \"ToplananMiktar\",T0.\"U_PaletNo\" as \"PaletNo\",(T2.\"OpenQty\" - ISNULL(T0.\"U_Miktar\", 0)) as \"PlanlananSiparisMiktari\",(SELECT Count(\"DocEntry\") FROM \"@AIF_WMS_KNTYNR1\" AS T98 WHERE T98.\"U_SiparisNo\" = T0.\"U_SiparisNumarasi\" and T98.\"U_SipSatirNo\" = T0.\"U_SiparisSatirNo\") AS \"KonteynerVarmi\",T0.\"U_TeslimatNo\",T0.\"U_BelgeNo\" AS \"BelgeNo\",T1.\"CardCode\" as \"MusteriKodu\",T1.\"CardName\" as \"MusteriAdi\",T0.\"DocEntry\" as \"ToplananDocEntry\" from \"@AIF_WMS_TOPLANAN\" as T0 WITH (NOLOCK) INNER JOIN ORDR AS T1 WITH (NOLOCK) ON T0.\"U_SiparisNumarasi\" = T1.\"DocEntry\" INNER JOIN RDR1 AS T2 WITH (NOLOCK) ON T1.\"DocEntry\" = T2.\"DocEntry\" and T2.\"LineNum\" = T0.\"U_SiparisSatirNo\") as tbl WHERE tbl.\"KonteynerVarmi\" <= 0 and tbl.\"BelgeNo\" = '" + docEntry + "' ";
        ...
        public Response getCekmeListesiMusterileri(string dbName, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    var query = "Select DISTINCT tbl.\"U_MusteriKodu\" as \"MusteriKodu\",tbl.\"U_MusteriAdi\" as \"MusteriAdi\" from (SELECT T0.\"U_MusteriKodu\",T0.\"U_MusteriAdi\",\"U_Aciklama\",(Select COUNT(T1.\"DocEntry\") from \"@AIF_WMS_SIPKAR1\" as T1 WITH (NOLOCK) where T1.\"DocEntry\" = T0.\"DocEntry\") as \"SatirSayisi\" FROM \"@AIF_WMS_SIPKAR\" AS T0 WITH (NOLOCK)) as tbl ";
                    try
                    {
                        using (SqlConnection con = new SqlConnection(connstring))
                        {
        ...
        public Response getCekmeListesiMusteriyeGoreUrunListesi(string dbName, string urunKodu, string mKod, string musterikod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    ...
                    #endregion old 2022.04.12 öncesi
                    ...
                    var query = " SELECT tbl.\"BelgeNo\" AS \"CekmeListesiNo\", T2.\"U_UrunKodu\" AS \"UrunKodu\",T2.\"U_UrunTanimi\" AS \"UrunTanimi\", tbl.\"Aciklama\", '-1' AS \"SiparisNumarasi\", '-1' AS \"SiparisSatirNo\", SUM(T2.\"U_PlanSipMik\") - SUM(T2.\"U_ToplananMik\") AS \"Miktar\", STRING_AGG(concat(T2.\"U_SiparisNumarasi\", '-', T2.\"U_SipSatirNo\"), ',') AS \"Kaynak\" FROM (SELECT T0.\"DocEntry\" AS \"BelgeNo\",\"U_Aciklama\" AS \"Aciklama\",T0.\"U_MuhKatGoster\", (SELECT COUNT(T1.\"DocEntry\") FROM \"@AIF_WMS_SIPKAR1\" AS T1 WITH (NOLOCK) WHERE T1.\"DocEntry\" = T0.\"DocEntry\") AS \"SatirSayisi\",T0.\"U_MusteriKodu\",T0.\"U_TerminalGizle\" FROM \"@AIF_WMS_SIPKAR\" AS T0 WITH (NOLOCK) WHERE T0.\"U_OnayDurumu\" = 'O') AS tbl INNER JOIN \"@AIF_WMS_SIPKAR1\" AS T2 ON tbl.\"BelgeNo\" = T2.\"DocEntry\" INNER JOIN OITM AS T3 WITH (NOLOCK) ON T3.\"ItemCode\" = T2.\"U_UrunKodu\" WHERE tbl.\"SatirSayisi\" > 0 AND(T2.\"U_UrunKodu\" = '" + urunKodu + "' OR T2.\"U_MuhKatNo\" = '" + urunKodu + "'  OR T3.\"CodeBars\" = '" + urunKodu + "' ) and tbl.\"U_MusteriKodu\"='" + musterikod + "' and  T2.U_Gorunur='Y' and " + condition + "(tbl.\"U_TerminalGizle\",'N') ='N'  GROUP BY tbl.\"BelgeNo\" ,T2.\"U_UrunKodu\",T2.\"U_UrunTanimi\", tbl.\"Aciklama\" ";
