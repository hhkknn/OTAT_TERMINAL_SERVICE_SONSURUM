    ...
    public class GetSonSatinalmaBelgesiNo
    {
        public Response getSonSatinalmaBelgesiNo(string dbName, string itemCode, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    var query = "Select MAX(T0.\"DocNum\") as \"DocNum\" from \"OPDN\" AS T0  WITH (NOLOCK) INNER JOIN \"PDN1\" AS T1 WITH (NOLOCK) ON T0.\"DocEntry\" = T1.\"DocEntry\" where T1.\"ItemCode\" = '" + itemCode + "' ";
