    ...
    public class GetBusinessPartner
    {
        public Response getBusinessVendorPartner(string dbName, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    var query = string.Format(@"select '' as 'CardCode', '' as 'CardName' UNION ALL select CardCode,  CardName as CardName from OCRD WITH (NOLOCK) where CardType='S' and ISNULL(validFor,'Y') = 'Y'");
                    try
                    {
        ...
        public Response getBusinessCustomerPartner(string dbName, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    var query = string.Format(@"select '' as 'CardCode', '' as 'CardName' UNION ALL select CardCode, CardName as CardName from OCRD WITH (NOLOCK) where CardType='C'");
                    try
                    {
                        using (SqlConnection con = new SqlConnection(connstring))
                        {
        ...
        public Response getAllBusinessPartner(string dbName, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    var query = "select '' as 'CardCode', '' as 'CardName' UNION ALL select CardCode, CardName as CardName from OCRD WITH (NOLOCK)";
                    try
                    {
                        using (SqlConnection con = new SqlConnection(connstring))
                        {
