        ...
        public Response getStokNakliFazlaMiktarKontrol(string dbName, List<string> docEntryList, List<string> WhsCodes, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    ...
                    query += "T0.\"OpenQty\" -ISNULL((Select SUM(T99.\"Quantity\") from DRF1 AS T99 WITH (NOLOCK) INNER JOIN ODRF AS T98 WITH (NOLOCK) ON T99.\"DocEntry\" = T98.\"DocEntry\" where T99.\"BaseLine\" = T0.\"LineNum\" and T98.\"DocStatus\" = 'O' and T99.\"LineStatus\" = 'O' and T99.\"BaseEntry\" = T0.\"DocEntry\"),0) as \"AcikMiktar\" from WTQ1 as T0 WITH (NOLOCK) ";
