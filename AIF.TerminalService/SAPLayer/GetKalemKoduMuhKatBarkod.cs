    ...
    public class GetKalemKoduMuhKatBarkod
    {
        public Response getKalemKoduMuhKatBarkod(string dbName, string itemCode, string cardCode, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    string query = "Select TOP 1 T0.\"ItemCode\" as \"Kalem Kodu\",T0.\"CodeBars\" as \"Barkod\",T1.\"Substitute\" as \"MuhatapKatalogNo\" from OITM as T0 WITH (NOLOCK) LEFT JOIN OSCN AS T1 WITH (NOLOCK) ON T0.\"ItemCode\" = T1.\"ItemCode\" ";
