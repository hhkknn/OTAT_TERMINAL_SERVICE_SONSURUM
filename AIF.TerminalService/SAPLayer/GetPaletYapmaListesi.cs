    ...
    public class GetPaletYapmaListesi
    {
        public Response getPaletYapmaListesi(string dbName, string pasifleriGoster, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    string query = "Select \"U_PaletNo\" as \"PaletNo\" from \"@AIF_WMS_PALET\" as T0 WITH (NOLOCK) where 1=1 ";
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    ...
                    else
                    {
                        ...
                        query += " GROUP BY T0.\"U_PaletNo\",T1.\"U_Barkod\",T1.\"U_MuhKatalogNo\",\"U_KalemKodu\",\"U_Tanim\",T0.\"U_ToplamKap\",T0.\"U_NetKilo\",T0.\"U_BrutKilo\",\"U_CekmeNo\",\"U_Kaynak\"";
                    }
                    try
                    {
                        using (SqlConnection con = new SqlConnection(connstring))
        ...
        public Response getPaletNoVarmi(string dbName, string paletNo, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    string query = "Select \"U_PaletNo\" from \"@AIF_WMS_PALET\" as T0 WITH (NOLOCK) where \"U_PaletNo\" = '" + paletNo + "' ";
