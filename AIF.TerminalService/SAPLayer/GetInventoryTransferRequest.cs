...
namespace AIF.TerminalService.SAPLayer
{
    public class GetInventoryTransferRequest
    {
        ...
        {
            DataTable dt = new DataTable();
            DataTable dtAlanlar = new DataTable(); //20231109
            try
            {
                GetConnectionString n = new GetConnectionString();
                string connstring = n.getConnectionString(dbName, mKod);

                #region otat ta veri getirmiyor bu yüzden terminal hata veriyor 20231109
                //if (connstring != "")
                //{
                ...
                //    }
                //    query += " (Select SUM(T99.\"Quantity\") from DRF1 AS T99 WITH (NOLOCK) INNER JOIN ODRF AS T98 WITH (NOLOCK) ON T99.\"DocEntry\" = T98.\"DocEntry\" where T99.\"BaseLine\" = T0.\"LineNum\" and T98.\"DocStatus\" = 'O' and T99.\"LineStatus\" = 'O' and T99.\"BaseEntry\" = T0.\"DocEntry\") as \"OnaydaBekleyenMiktar\" ";
                ...
                //    else if (mKod == "30TRMN")
                //    {
                //        query += "T0.\"U_KaynakDYeri\" as \"DepoYeriAdi\",T0.\"U_HedefDYeri\" as \"HDepoYeriAdi\",(SELECT T67.\"AbsEntry\" FROM OBIN as T67 WITH (NOLOCK) where T67.\"BinCode\" = T0.\"U_KaynakDYeri\") as \"DepoYeriId\",(SELECT T77.\"AbsEntry\" FROM OBIN as T77 where T77.\"BinCode\" = T0.\"U_HedefDYeri\") as \"HDepoYeriId\",";
                //    }
                //    query += "(Select \"WhsName\" from \"OWHS\" as t99 WITH (NOLOCK) where t99.\"WhsCode\" = T0.\"WhsCode\") as \"HedefDepoAdi\",(Select \"WhsName\" from \"OWHS\" as t99 where t99.\"WhsCode\" = T0.\"FromWhsCod\") as \"DepoAdi\" from WTQ1 as T0 ";

                //    query += "INNER JOIN OITM as T1 WITH (NOLOCK) ON T0.ItemCode = T1.ItemCode ";
                //    query += "INNER JOIN OITW as T2 WITH (NOLOCK) ON T1.\"ItemCode\" = T2.\"ItemCode\" and T2.\"WhsCode\" = T0.\"FromWhsCod\" ";
                //    query += "INNER JOIN OWTQ as T4 WITH (NOLOCK) ON T0.\"DocEntry\" = T4.\"DocEntry\" ";

                //    query += "INNER JOIN \"@AIF_WMS_USRWHS\" AS T34 WITH (NOLOCK) ON T34.\"U_KullaniciKodu\" = '" + kullaniciKodu + "' "; //giren kullanıcıya göre filtrelemesi için eklendi 02.06.2022 chn
                ...
                #region son versiyon hali
                if (connstring != "")
                {
                    string ss = "SELECT \"TABLE_NAME\",\"COLUMN_NAME\" FROM INFORMATION_SCHEMA.COLUMNS ";
                    try
                    {
                        using (SqlConnection con = new SqlConnection(connstring))
                        {
                            using (SqlCommand cmd = new SqlCommand(ss, con))
                            {
                                cmd.CommandType = CommandType.Text;
                                using (SqlDataAdapter sda = new SqlDataAdapter(cmd))
                                {
                                    using (dtAlanlar = new DataTable())
                                    {
                                        sda.Fill(dtAlanlar);
                                        dt.TableName = "DBAlanlar";
                                    }
                                }
                            }
                        }
                    ...
                    var query = "select T0.DocEntry as [BelgeNumarasi], T0.ItemCode as [KalemKodu], T0.Dscription as [KalemAdi], T0.OpenQty as [Miktar], T0.UomCode as [OlcuBirimi],T5.UomName AS[OlcuBirimiAdi], T1.\"ManBtchNum\" as [Partili],T1.\"ManSerNum\" as [Serili],T1.\"CodeBars\" as [Barkod],T0.\"FromWhsCod\" as \"DepoKodu\",T0.\"WhsCode\" as \"HedefDepo\",\"LineNum\" as \"SiraNo\",T2.\"OnHand\" as \"DepoMiktar\" ";
                    ...
                    if (mKod == "10TRMN" || mKod == "20TRMN")
                    {
                        query += "cast('' as varchar(254)) as \"DepoYeriId\",cast('' as varchar(254)) as \"DepoYeriAdi\",cast('' as varchar(254)) as \"HDepoYeriAdi\",cast('' as varchar(254)) as \"HDepoYeriId\",";
                    }
                    else if (mKod == "30TRMN")
                    {
                    ...
                    query += "(Select \"WhsName\" from \"OWHS\" as t99 where t99.\"WhsCode\" = T0.\"WhsCode\") as \"HedefDepoAdi\",(Select \"WhsName\" from \"OWHS\" as t99 where t99.\"WhsCode\" = T0.\"FromWhsCod\") as \"DepoAdi\",";

                    if (dtAlanlar.AsEnumerable().Where(x => x.Field<string>("TABLE_NAME") == "OSCN" && x.Field<string>("COLUMN_NAME") == "U_KalemKodu").Count() > 0)
                    {
                        query += "(Select TOP 1 T5.\"Substitute\" from OSCN AS T5 where T5.\"U_KalemKodu\" = T1.\"ItemCode\") ";
                    }
                    else
                    {
                        query += "''";
                    ...
                    query += " as \"MuhatapKatalogNo\"";



                    query += " from WTQ1 as T0 ";
                    ...
                    query += "LEFT JOIN OUOM AS T5 ON T0.UomCode = T5.UomCode ";
                ...
                if (connstring != "")
                {
                    ...
                    if (WhsCodes != null)
                    {
                        ...
                        if (WhsCodes.Count > 0)
                        {
                            ...
                            {
                                query += " ) "; // values boş olduğunda yuakrıdaki " and (T1.FromWhsCod IN (" + values + ") "; sorgusunun parantesi kapanmadığı için sorgu hata veriyordu, bu else ksımını ekledim chn 202201103
                            }
                        }
                        else
                        {
