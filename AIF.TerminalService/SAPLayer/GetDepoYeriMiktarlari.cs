        ...
        public Response getDepoYeriMiktarlari(string dbName, string urunKod, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    ...
                    var query = "SELECT T0.\"AbsEntry\" as \"AbsEntry\",T0.\"BinCode\" as \"Depo Yeri\", T1.\"ItemCode\" as \"Ürün Kodu\", T2.\"ItemName\" as \"Ürün Adı\", T1.\"OnHandQty\" as \"Miktar\" FROM \"OBIN\" T0 WITH (NOLOCK) LEFT JOIN \"OIBQ\" T1 WITH (NOLOCK) ON T0.\"WhsCode\" = T1.\"WhsCode\" AND T0.\"AbsEntry\" = T1.\"BinAbs\" LEFT JOIN \"OITM\" T2 ON T1.\"ItemCode\" = T2.\"ItemCode\"  where T1.\"ItemCode\" ='" + urunKod + "' ";
