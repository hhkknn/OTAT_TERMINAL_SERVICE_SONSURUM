    ...
    public class GetWareHouse
    {
        public Response getWareHouse(string dbName, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    var query = string.Format(@"select '' as 'WhsCode', '' as 'WhsName' UNION ALL select WhsCode, WhsName from OWHS WITH (NOLOCK) where Inactive='N'");
                    try
                    {
        ...
        public Response getWareHouseByUserCode(string dbName, string kullaniciId, string belgeTipi, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    ...
                    if (belgeTipi == "")
                    {
                        query = "select '' as 'WhsCode', '' as 'WhsName','' as \"DepoYeriZorunlu\" UNION ALL Select \"U_DepoKodu\" as 'WhsCode',\"U_DepoAdi\" as 'WhsName',T2.\"BinActivat\" as \"DepoYeriZorunlu\"  from \"@AIF_WMS_USRWHS\" as T0 WITH (NOLOCK) INNER JOIN \"@AIF_WMS_USRWHS1\" as T1 WITH (NOLOCK) ON T0.\"DocEntry\"=T1.\"DocEntry\" LEFT JOIN OWHS AS T2 ON T1.\"U_DepoKodu\" = T2.\"WhsCode\" where T0.\"U_KullaniciKodu\" = '" + kullaniciId + "' and \"U_Secim\" = 'Y'";
                    }
                    else
                    {
                        query = "select '' as \"WhsCode\", '' as \"WhsName\",'' as \"DepoYeriZorunlu\",'' as \"Secim\",'' as \"TamYetki\",'' as \"SipMalGrs\",'' as \"U_BlgszMalGrs\",'' as \"TlpszDepK\",'' as \"TlpszDepH\",'' as \"TlpBagDepK\",'' as \"TlpBagDepH\",'' as \"TlpKabulK\",'' as \"TlpKabulH\",'' as \"BlgszMalC\",'' as \"SipBagTes\",'' as \"SprsszTes\",'' as \"TeslmtIade\",'' as \"SatisIade\" UNION ALL ";
                        query += "select * from (";
                        query += " Select ";
                        ...
                        query += "from \"@AIF_WMS_USRWHS\" as T0 WITH (NOLOCK) ";
                        query += "INNER JOIN \"@AIF_WMS_USRWHS1\" as T1 WITH (NOLOCK) ON T0.\"DocEntry\" = T1.\"DocEntry\" ";
                        query += "LEFT JOIN OWHS AS T2 WITH (NOLOCK) ON T1.\"U_DepoKodu\" = T2.\"WhsCode\" ";
        ...
        public Response getWareHouseDetailsWithQty(string dbName, string warehouse, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    var query = "SELECT T1.[ItemCode] as \"Kalem Kodu\", T1.[ItemName] as \"Kalem Adı\", T0.[OnHand] \"Stokta\", T2.[UomCode] as \"Birim\" FROM [dbo].[OITW]  T0 WITH (NOLOCK) INNER JOIN OITM T1 WITH (NOLOCK) ON T0.[ItemCode] = T1.[ItemCode]  LEFT JOIN OUOM T2 WITH (NOLOCK) ON T1.InvntryUom = T2.UomName where 1=1";
                    if (warehouse != "")
                    {
        ...
        public Response getWareHouseByUserCodeAddName(string dbName, string kullaniciId, string belgeTipi, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    ...
                    if (belgeTipi == "")
                    {
                        query = "select '' as 'WhsCode', '' as 'WhsName' UNION ALL Select \"U_DepoKodu\" as 'WhsCode', (ISNULL(\"U_DepoKodu\",'')+'-'+ISNULL(\"U_DepoAdi\",'')) as 'WhsName'  from \"@AIF_WMS_USRWHS\" as T0 WITH (NOLOCK) INNER JOIN \"@AIF_WMS_USRWHS1\" as T1 WITH (NOLOCK) ON T0.\"DocEntry\"=T1.\"DocEntry\" LEFT JOIN OWHS AS T2 WITH (NOLOCK) ON T1.\"U_DepoKodu\" = T2.\"WhsCode\" where T0.\"U_KullaniciKodu\" = '" + kullaniciId + "' and \"U_Secim\" = 'Y'";
                    }
                    else
                    {
