        ...
        public Response getOrdersDetails(string dbName, List<string> docEntryList, List<string> WhsCodes, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    ...
                    query += "T3.\"NumAtCard\" as[MusteriSipNo], T3.\"ShipToCode\" as [SevkiyatAdresi],(Select t99.\"WhsName\" from OWHS t99 where t99.\"WhsCode\" = T0.\"WhsCode\") as \"DepoAdi\" from RDR1 as T0 WITH (NOLOCK) INNER JOIN OITM as T1 WITH (NOLOCK) ON T0.ItemCode = T1.ItemCode INNER JOIN OITW as T2 ON T1.\"ItemCode\" = T2.\"ItemCode\" INNER JOIN ORDR as T3 WITH (NOLOCK) ON T0.\"DocEntry\" = T3.\"DocEntry\" and T2.\"WhsCode\" = T0.\"WhsCode\"  where T0.DocEntry IN (" + docentryvalList + ") and T0.OpenQty>0";
