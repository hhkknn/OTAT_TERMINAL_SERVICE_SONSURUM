        ...
        public Response getCekmeListesiKontrol(string dbName, string belgeNo, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    ...
                    query = "SELECT  T0.\"DocEntry\",T1.\"U_PlanSipMik\",ISNULL((SELECT  SUM(T3.\"U_Miktar\") FROM \"@AIF_WMS_TOPLANAN\" T3 WITH (NOLOCK)";
                    query += "WHERE T3.\"U_SiparisNumarasi\" = T1.\"U_SiparisNumarasi\" AND T3.\"U_SiparisSatirNo\" = T1.\"U_SipSatirNo\" AND T3.\"U_KalemKodu\" = T1.\"U_UrunKodu\" AND T3.\"U_BelgeNo\" = T0.\"DocEntry\"), 0 ) AS \"Toplanan\", T1.\"U_PlanSipMik\" - ISNULL((SELECT  SUM(T3.\"U_Miktar\") FROM \"@AIF_WMS_TOPLANAN\" T3 WITH (NOLOCK)";
                    query += "WHERE T3.\"U_SiparisNumarasi\" = T1.\"U_SiparisNumarasi\" AND T3.\"U_SiparisSatirNo\" = T1.\"U_SipSatirNo\" AND T3.\"U_KalemKodu\" = T1.\"U_UrunKodu\" AND T3.\"U_BelgeNo\" = T0.\"DocEntry\"), 0 ) AS \"Kalan\", T1.\"U_SipSatirNo\", T1.\"U_SiparisNumarasi\", T1.\"U_UrunKodu\" FROM \"@AIF_WMS_SIPKAR\" T0 WITH (NOLOCK)";
                    query += "INNER JOIN \"@AIF_WMS_SIPKAR1\" T1 WITH (NOLOCK) ON T0.\"DocEntry\" = T1.\"DocEntry\" ";
