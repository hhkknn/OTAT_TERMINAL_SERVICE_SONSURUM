            ...
            try
            {
                ...
                if (connstring != "")
                {
                    ...
                    foreach (var item in docEntryList)
                    {
                        ...
                        var query = "SELECT T1.\"DocEntry\" as 'Fatura No',T1.\"ItemCode\" as 'Kalem Kodu', T1.\"Dscription\" as 'Kalem Adı',T1.\"UomCode\" as 'Birim',T1.\"OpenCreQty\" as 'Miktar' ,\"LineNum\" as 'Sıra' FROM ODLN T0 WITH (NOLOCK) INNER JOIN DLN1 T1 WITH (NOLOCK) ON T0.[DocEntry] = T1.[DocEntry] WHERE T0.DocEntry IN (" + docentryvalList + ")and T1.\"LineStatus\" = 'O' AND T0.DocType = 'I'";
