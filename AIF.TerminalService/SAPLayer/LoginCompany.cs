        ...
        public Response Connect(string userName, string password, string dbName, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    ...
                    if (mKod != "70TRMN")
                    {
                        query = "select Cast(\"Code\" as varchar(10)) as \"Kod\",\"ExtEmpNo\" as \"Pass\",\"firstName\" as Ad,\"lastName\" as Soyad from OHEM WITH (NOLOCK) where Cast(\"empID\" as varchar(10)) = '" + userName + "' and \"ExtEmpNo\"='" + password + "' and \"Active\" = 'Y' "; //empID eşleştirmesi yerine Code yapıldı.
                    }
                    else
                    {
                        query = "select Cast(\"empID\" as varchar(10)) as \"Kod\",\"ExtEmpNo\" as \"Pass\",\"firstName\" as Ad,\"lastName\" as Soyad from OHEM WITH (NOLOCK) where Cast(\"empID\" as varchar(10)) = '" + userName + "' and \"ExtEmpNo\"='" + password + "' and \"Active\" = 'Y' "; //empID eşleştirmesi yerine Code yapıldı.
                    }
                    try
                    {
                        using (SqlConnection con = new SqlConnection(connstring))
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            query = "Select TOP 1 * from \"@AIF_WMS_CONSTRNG\" WITH (NOLOCK)";
