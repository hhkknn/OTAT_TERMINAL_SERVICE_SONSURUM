        ...
        public Response deletePaletListesi(string dbName, List<SilinecekPaletler> silinecekPaletlers, string cekmeListesiKalemleriniGrupla)
        {
            ...
            try
            {
                ...
                foreach (var item in silinecekPaletlers)
                {
                    ...
                    if (cekmeListesiKalemleriniGrupla != "Y")
                    {
                        sql = "DELETE FROM \"@AIF_WMS_TOPLANAN\" where \"U_PaletNo\" = '" + item.paletNo + "' and \"U_SiparisNumarasi\" = '" + item.siparisNo + "' and \"U_SiparisSatirNo\" = '" + item.siparisSatirNo + "' \"U_BelgeNo\" = '" + item.cekmeNo + "' ";
                    }
                    else
                    {
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i <= cekmeNoListesi.Count() - 1; i++)
                        {
                            ...
                            sql = "Select \"LineId\",\"U_ToplananMik\" FROM \"@AIF_WMS_SIPKAR1\" t0 WITH (NOLOCK) WHERE \"DocEntry\" = '" + item.cekmeNo + "' and \"U_UrunKodu\" = '" + item.kalemKodu + "' and CONCAT(\"U_SiparisNumarasi\", '-', \"U_SipSatirNo\")  = '" + cekmeNoListesi[i].ToString() + "'";
