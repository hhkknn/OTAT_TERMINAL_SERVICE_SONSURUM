    ...
    public class GetBatchNumber
    {
        public Response getBatchNumberByCardCodeAndItemCode(string dbName, string CardCode, string ItemCode, string mKod)
        {
            ...
            try
            {
                ...
                if (connstring != "")
                {
                    var query = "SELECT ROW_NUMBER() OVER(ORDER BY T0.BatchNum) AS ID,T0.BatchNum, SUM(T0.Quantity) as Quantity FROM IBT1 T0 WITH (NOLOCK) WHERE T0.CardCode ='" + CardCode + "' and T0.ItemCode='" + ItemCode + "' and (T0.BaseType = '13'  or T0.BaseType = '15') group by T0.BatchNum ";

                    if (CardCode == "")
                    {
                        query = "SELECT ROW_NUMBER() OVER(ORDER BY T0.BatchNum) AS ID,T0.BatchNum, SUM(T0.Quantity) as Quantity FROM IBT1 T0 WITH (NOLOCK) WHERE T0.ItemCode='" + ItemCode + "' group by T0.BatchNum  ";
                    }
